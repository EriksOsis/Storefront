...
export function Nav(props) {
    ...
    return (
        <div className={classes.all}>
            <nav>
                <Categories onClose={hideCartHandler}/>
                ...
                <div className={classes['nav-icons']}>
                    <button className={classes['select-btn']} onClick={props.onOpen}>{props.currencyId}
                        <svg className={props.arrowClasses} width="8" height="4" viewBox="0 0 8 4" fill="none"
                             xmlns="http://www.w3.org/2000/svg">
                            <path d="M1 0.5L4 3.5L7 0.5" stroke="black" strokeLinecap="round" strokeLinejoin="round"/>
                        </svg>
                    </button>
                    <div className={props.dropDownClasses}>
                        {data.currencies.map((currency) =>
                            <button key={currency.label} className={classes['option-btn']} onClick={props.onConvert}
                    ...
                    <div className={classes.cart}>
                        <svg className={classes['cart-btn']}
                             onClick={numOfCartItems !== 0 ? showCartHandler : undefined} width="20" height="20"
            ...
            {cartIsShown &&
                <Cart numOfCartItems={numOfCartItems} currencyId={props.currencyId} onClose={hideCartHandler}/>}
        </div>
    )
}