...
export function ProductItem(props) {
    ...
    const attributes = props.attributes.map((attribute) => (attribute.items.map((item) => item.value))[0]).join('&');
    const productId = `${props.id}_${attributes}`;
    // console.log(productId.substr(props.id.length + 1).split('&'));
    // console.log(productId.substr(props.id.length + 1));
    ...
    return (
        <div key={props.id}
             className={props.inStock ? classes['prod-card'] : classes['prod-card-oos']}>
            <Link to={'/product/' + props.id} className={classes.fix}>
                <img className={classes['prod-img']} src={props.gallery[0]} alt={props.name}/>
                <p className={classes.title}>{`${props.brand} ${props.name}`}</p>
                <p className={classes.price}>
                    <strong>
                        {props.prices.map((currency) => (
                            currency.currency.symbol === props.currencyId) && (props.currencyId + currency.amount))
                        }
                    </strong>
                </p>
            </Link>
            <button onClick={onAddToCartHandler} className={classes['quick-add']}>
