...
export class ProductItem extends React.Component {
    ...
    onAddToCartHandler() {
        ...
        const attributes = this.props.attributes.map((attribute) => (attribute.items.map((item) => item.value))[0]).join('&');
        const productId = `${this.props.id}_${attributes}`;
    ...
    render() {
        return (
            <div key={this.props.id}
                 className={this.props.inStock ? classes['prod-card'] : classes['prod-card-oos']}>
                <Link to={'/product/' + this.props.id} className={classes.fix}>
                    <img className={classes['prod-img']} src={this.props.gallery[0]} alt={this.props.name}/>
                ...
                <button onClick={this.onAddToCartHandler} className={classes['quick-add']}>
